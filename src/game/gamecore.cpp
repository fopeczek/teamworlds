...
void CCharacterCore::Init(CWorldCore *pWorld, CCollision *pCollision, int Team, int MapID, CPlayerClass player_Class) {
    m_pWorld = pWorld;
    m_pCollision = pCollision;
    m_Team = Team;
    m_MapID = MapID;
    m_PlayerClass = player_Class;
}
...
void CCharacterCore::Tick(bool UseInput, bool &doReveal, AvailableCheats *pCheats) {
    ...
    if (m_PlayerClass.IsClass(Class::Tank)) {
        Accel = Grounded ? TANK_GROUND_ACCELERATION : TANK_AIR_ACCELERATION;
    }
                ...
                if (m_Input.m_Jump) {
                    if (!(m_Jumped & 1)) {
                        if (Grounded) {
                            ...
                            m_Vel.y = m_PlayerClass.IsClass(Class::Tank) ? -m_pWorld->m_Tuning.m_GroundJumpImpulse *
                                                                           TANK_JUMP_FACTOR
                                                                         : -m_pWorld->m_Tuning.m_GroundJumpImpulse;
                            m_Jumped |= 1;
                        } else if (!(m_Jumped & 2)) {
                            m_TriggeredEvents |= COREEVENTFLAG_AIR_JUMP;
                            m_Vel.y = m_PlayerClass.IsClass(Class::Tank) ? -m_pWorld->m_Tuning.m_AirJumpImpulse *
                                                                           TANK_JUMP_FACTOR
                                                                         : -m_pWorld->m_Tuning.m_AirJumpImpulse;
                            if (!pCheats->Jetpack)
                                m_Jumped |= 3;
                        }
                    }
    ...
    if (m_HookState == HOOK_IDLE) {
        ...
        if (Hit) {
            if (Hit & CCollision::COLFLAG_NOHOOK)
                if (pCheats) {
                    if (pCheats->SuperHook or m_PlayerClass.IsClass(Class::Spider)) {
    ...
    if (m_HookState == HOOK_GRABBED) {
        if (m_HookedPlayer != -1) {
            ...
            if (pCharCore) {
                ...
                if (pCharCore->m_Team != m_Team and m_Team != -1 and pCharCore->m_PlayerClass.IsClass(Class::Hunter)) {
        ...
        if (m_HookedPlayer == -1 && distance(m_HookPos, m_Pos) > 46.0f) {
            ...
            if (m_PlayerClass.IsClass(Class::Tank)) {
                HookVel.y *= TANK_HOOK_Y_FACTOR;
            }

            if (m_PlayerClass.IsClass(Class::Tank)) {
                HookVel.x *= TANK_HOOK_X_FACTOR;
            }
        ...
        if (pCheats) {
            if (!pCheats->SuperHook and !m_PlayerClass.IsClass(Class::Spider)) {
                m_HookTick++;
            }
        } else {
            if (m_PlayerClass.IsClass(Class::Spider)) {
                m_HookTick++;
            }
        }
        if (m_HookedPlayer != -1 &&
    ...
    if (m_pWorld) {
        for (int i = 0; i < MAX_CLIENTS; i++) {
            ...
            if (m_pWorld->m_Tuning.m_PlayerCollision && Distance < PHYS_SIZE * 1.25f && Distance > 0.0f and
                !m_PlayerClass.IsClass(Class::Tank)) {
            ...
            if (m_HookedPlayer == i && m_pWorld->m_Tuning.m_PlayerHooking) {
                if (Distance > PHYS_SIZE * 1.50f) // TODO: fix tweakable variable
                {
                    ...
                    if (pCharCore->m_PlayerClass.IsClass(Class::Tank)) {
                        pCharCore->m_HookDragVel *= TANK_HOOK_DRAG_FACTOR;
                    }
                }
            }
        }
...
void CCharacterCore::Move() {
    ...
    float RampValue = VelocityRamp(length(m_Vel) * 50, m_pWorld->m_Tuning.m_VelrampStart,
                                   m_pWorld->m_Tuning.m_VelrampRange, m_pWorld->m_Tuning.m_VelrampCurvature);
    ...
    if (m_pWorld->m_Tuning.m_PlayerCollision) {
        ...
        for (int i = 0; i < End; i++) {
            ...
            for (int p = 0; p < MAX_CLIENTS; p++) {
                ...
                if (!pCharCore || pCharCore == this || this->m_MapID != pCharCore->m_MapID)
                    continue;
