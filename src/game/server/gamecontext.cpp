...
void
CGameContext::CreateDamage(vec2 Pos, int Id, vec2 Source, int HealthAmount, int ArmorAmount, bool Self, int MapID) {
    float f = angle(Source);
    CNetEvent_Damage *pEvent = (CNetEvent_Damage *) m_Events.Create(NETEVENTTYPE_DAMAGE, sizeof(CNetEvent_Damage), -1,
                                                                    MapID);
    if (pEvent) {
...
void CGameContext::CreateHammerHit(vec2 Pos, int MapID) {
    ...
    CNetEvent_HammerHit *pEvent = (CNetEvent_HammerHit *) m_Events.Create(NETEVENTTYPE_HAMMERHIT,
                                                                          sizeof(CNetEvent_HammerHit), -1, MapID);
    if (pEvent) {
        pEvent->m_X = (int) Pos.x;
        pEvent->m_Y = (int) Pos.y;
    }
...
void CGameContext::CreateExplosion(vec2 Pos, int Owner, int Weapon, int MaxDamage, int MapID) {
    ...
    CNetEvent_Explosion *pEvent = (CNetEvent_Explosion *) m_Events.Create(NETEVENTTYPE_EXPLOSION,
                                                                          sizeof(CNetEvent_Explosion), -1, MapID);
    if (pEvent) {
        pEvent->m_X = (int) Pos.x;
        pEvent->m_Y = (int) Pos.y;
    }
...
void CGameContext::CreateDeath(vec2 Pos, int ClientID, int MapID) {
    // create the event
    CNetEvent_Death *pEvent = (CNetEvent_Death *) m_Events.Create(NETEVENTTYPE_DEATH, sizeof(CNetEvent_Death), -1,
                                                                  MapID);
    if (pEvent) {
        pEvent->m_X = (int) Pos.x;
        pEvent->m_Y = (int) Pos.y;
...
void CGameContext::CreateSound(vec2 Pos, int Sound, int64 Mask, int MapID) {
    ...
    CNetEvent_SoundWorld *pEvent = (CNetEvent_SoundWorld *) m_Events.Create(NETEVENTTYPE_SOUNDWORLD,
                                                                            sizeof(CNetEvent_SoundWorld), Mask, MapID);
    if (pEvent) {
        pEvent->m_X = (int) Pos.x;
        pEvent->m_Y = (int) Pos.y;
...
void CGameContext::OnTick() {
    ...
    if (m_VoteCloseTime) {
        ...
        if (m_VoteCloseTime == -1)
            ...
            } else if (m_VoteEnforce == VOTE_CHOICE_NO || (m_VoteUpdate && No >= (Total + 1) / 2) ||
...
void CGameContext::OnClientEnter(int ClientID) {
    ...
    NewClientInfoMsg.m_ClassID = GetClassID(Server()->GetClientClass(ClientID).m_Class);
    ...
    for (int i = 0; i < MAX_CLIENTS; ++i) {
        ...
        ClientInfoMsg.m_ClassID = GetClassID(Server()->GetClientClass(i).m_Class);
...
void CGameContext::OnMessage(int MsgID, CUnpacker *pUnpacker, int ClientID) {
    ...
    if (Server()->ClientIngame(ClientID)) {
        if (MsgID == NETMSGTYPE_CL_SAY) {
            if (Config()->m_SvSpamprotection && pPlayer->m_LastChatTeamTick &&
                pPlayer->m_LastChatTeamTick + Server()->TickSpeed() > Server()->Tick())
                return;
            ...
            if (Length == 0 || (Config()->m_SvSpamprotection && pPlayer->m_LastChatTeamTick &&
                                pPlayer->m_LastChatTeamTick + Server()->TickSpeed() * (Length / 20) > Server()->Tick()))
                return;
            ...
            if (pMsg->m_Force) {
                ...
                if ((Config()->m_SvSpamprotection &&
                     ((pPlayer->m_LastVoteTryTick && pPlayer->m_LastVoteTryTick + Server()->TickSpeed() * 3 > Now) ||
                      (pPlayer->m_LastVoteCallTick &&
                       pPlayer->m_LastVoteCallTick + Server()->TickSpeed() * VOTE_COOLDOWN > Now))) ||
            ...
            if (pPlayer->GetTeam() == pMsg->m_Team ||
                (Config()->m_SvSpamprotection && pPlayer->m_LastSetTeamTick &&
                 pPlayer->m_LastSetTeamTick + Server()->TickSpeed() * 3 > Server()->Tick()) ||
            ...
            if (Config()->m_SvSpamprotection && pPlayer->m_LastSetSpectatorModeTick &&
                pPlayer->m_LastSetSpectatorModeTick + Server()->TickSpeed() > Server()->Tick())
                return;
            ...
            if (Config()->m_SvSpamprotection && pPlayer->m_LastEmoteTick &&
                pPlayer->m_LastEmoteTick + Server()->TickSpeed() * 3 > Server()->Tick())
                return;
            ...
            if (pPlayer->m_LastReadyChangeTick &&
                pPlayer->m_LastReadyChangeTick + Server()->TickSpeed() * 1 > Server()->Tick())
                return;

            pPlayer->m_LastReadyChangeTick = Server()->Tick();
            m_pController->OnPlayerReadyChange(pPlayer);
        } else if (MsgID == NETMSGTYPE_CL_SKINCHANGE) {
            if (pPlayer->m_LastChangeInfoTick &&
                pPlayer->m_LastChangeInfoTick + Server()->TickSpeed() * 5 > Server()->Tick())
                return;
            ...
            for (int i = 0; i < MAX_CLIENTS; ++i) {
                if (!m_apPlayers[i] || (!Server()->ClientIngame(i) && !m_apPlayers[i]->IsDummy()) ||
                    Server()->GetClientVersion(i) < MIN_SKINCHANGE_CLIENTVERSION)
                    continue;
...
void CGameContext::ConSetClass(IConsole::IResult *pResult, void *pUserData) {
    ...
    if (player) {
        if (player->GetCharacter()) {
            if (!pSelf->Server()->GetClientClass(player->GetCID()).IsClass(wanted_class)) {
...
void CGameContext::OnConsoleInit() {
    ...
    Console()->Register("tune", "s[tuning] ?i[value]", CFGFLAG_SERVER, ConTuneParam, this,
                        "Tune variable to value or show current value");
    Console()->Register("tune_reset", "?s[tuning]", CFGFLAG_SERVER, ConTuneReset, this,
    ...
    Console()->Register("Godmode", "?i[playerID]", CFGFLAG_SERVER, ConGodmode, this,
                        "Toggle godmode for certain player");
    Console()->Register("AllWeapons", "?i[playerID]", CFGFLAG_SERVER, ConAllWeapons, this,
                        "Toggle all weapon availability for certain player");
    Console()->Register("FullAuto", "?i[playerID]", CFGFLAG_SERVER, ConFullAuto, this,
                        "Toggle full auto fire mode for certain player");
    Console()->Register("NoSelfDamage", "?i[playerID]", CFGFLAG_SERVER, ConNoSelfDamage, this,
                        "Toggle no self damage mode for certain player");
    Console()->Register("NoEnemyDamage", "?i[playerID]", CFGFLAG_SERVER, ConNoEnemyDamage, this,
                        "Toggle no enemy damage mode for certain player");
    Console()->Register("Jetpack", "?i[playerID]", CFGFLAG_SERVER, ConJetpack, this,
                        "Toggle jetpack mode for certain player");
    Console()->Register("SuperHook", "?i[playerID]", CFGFLAG_SERVER, ConSuperHook, this,
                        "Toggle jetpack mode for certain player");
    Console()->Register("SuperNinja", "?i[playerID]", CFGFLAG_SERVER, ConSuperNinja, this,
                        "Toggle super ninja mode for certain player");
    Console()->Register("LockMovement", "?i[playerID]", CFGFLAG_SERVER, ConLockMovement, this,
                        "Toggle lock for movement of certain player");
    Console()->Register("LockHook", "?i[playerID]", CFGFLAG_SERVER, ConLockHook, this,
                        "Toggle lock for hook of certain player");
    Console()->Register("LockWeapons", "?i[playerID]", CFGFLAG_SERVER, ConLockWeapons, this,
                        "Toggle lock for weapons of certain player");
    Console()->Register("LockPosition", "?i[playerID]", CFGFLAG_SERVER, ConLockPosition, this,
                        "Toggle lock of position for certain player");
    Console()->Register("ResetCheats", "?i[playerID]", CFGFLAG_SERVER, ConResetCheats, this,
                        "Reset all cheats for certain player");
    Console()->Register("ResetLocks", "?i[playerID]", CFGFLAG_SERVER, ConResetLocks, this,
                        "Reset all cheats for certain player");
    ...
    Console()->Register("clearWalls", "?i[your id]", CFGFLAG_SERVER, ConRemoveWalls, this,
                        "Manually remove all walls and webs");
