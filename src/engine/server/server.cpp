...
void CServerBan::InitServerBan(IConsole *pConsole, IStorage *pStorage, CServer *pServer) {
    ...
    Console()->Register("ban", "s[id|ip|range] ?i[minutes] r[reason]", CFGFLAG_SERVER | CFGFLAG_STORE, ConBanExt, this,
...
void CServer::SetClientName(int ClientID, const char *pName) {
    ...
    str_utf8_copy_num(m_aClients[ClientID].m_aName, *pName ? pName : pDefaultName, sizeof(m_aClients[ClientID].m_aName),
                      MAX_NAME_LENGTH);
}
...
void CServer::SetClientClass(int ClientID, Class who) {
    m_aClients[ClientID].m_PlayerClass.m_Class = who;
}

CPlayerClass CServer::GetClientClass(int ClientID) {
    return m_aClients[ClientID].m_PlayerClass;
}
...
int CServer::SendMsg(CMsgPacker *pMsg, int Flags, int ClientID) {
    ...
    // drop invalid packet
    if (ClientID != -1 &&
        (ClientID < 0 || ClientID >= MAX_CLIENTS || m_aClients[ClientID].m_State == CClient::STATE_EMPTY ||
         m_aClients[ClientID].m_Quitting))
        return 0;
...
int CServer::NewClientCallback(int ClientID, void *pUser) {
    ...
    pThis->m_aClients[ClientID].m_PlayerClass.m_Class = Class::None;
...
void CServer::UpdateClientRconCommands() {
    for (int ClientID = Tick() % MAX_RCONCMD_RATIO; ClientID < MAX_CLIENTS; ClientID += MAX_RCONCMD_RATIO) {
        if (m_aClients[ClientID].m_State != CClient::STATE_EMPTY && m_aClients[ClientID].m_Authed) {
            int ConsoleAccessLevel = m_aClients[ClientID].m_Authed == AUTHED_ADMIN ? IConsole::ACCESS_LEVEL_ADMIN
                                                                                   : IConsole::ACCESS_LEVEL_MOD;
            for (int i = 0; i < MAX_RCONCMD_SEND && m_aClients[ClientID].m_pRconCmdToSend; ++i) {
                SendRconCmdAdd(m_aClients[ClientID].m_pRconCmdToSend, ClientID);
                m_aClients[ClientID].m_pRconCmdToSend = m_aClients[ClientID].m_pRconCmdToSend->NextCommandInfo(
                        ConsoleAccessLevel, CFGFLAG_SERVER);
            }
        }
    }
}
...
void CServer::UpdateClientMapListEntries() {
    for (int ClientID = Tick() % MAX_RCONCMD_RATIO; ClientID < MAX_CLIENTS; ClientID += MAX_RCONCMD_RATIO) {
        if (m_aClients[ClientID].m_State != CClient::STATE_EMPTY && m_aClients[ClientID].m_Authed &&
            ...
            for (int i = 0;
                 i < MAX_MAPLISTENTRY_SEND && m_aClients[ClientID].m_MapListEntryToSend < m_lMaps.size(); ++i) {
        ...
        if (Unpacker.Type() == NETMSG_INFO) {
            if ((pPacket->m_Flags & NET_CHUNKFLAG_VITAL) != 0 && m_aClients[ClientID].m_State == CClient::STATE_AUTH) {
                ...
                if (str_comp(pVersion, GameServer()->NetVersion()) != 0) {
                    // wrong version
                    char aReason[256];
                    str_format(aReason, sizeof(aReason), "Wrong version. Server is running '%s' and client '%s'",
                               GameServer()->NetVersion(), pVersion);
            ...
            if ((pPacket->m_Flags & NET_CHUNKFLAG_VITAL) != 0 &&
                (m_aClients[ClientID].m_State == CClient::STATE_CONNECTING ||
                 m_aClients[ClientID].m_State == CClient::STATE_CONNECTING_AS_SPEC)) {
                int ChunkSize = MAP_CHUNK_SIZE;

                // send map chunks
                for (int i = 0;
                     i < m_vMapData[MapID].m_MapChunksPerRequest && m_aClients[ClientID].m_MapChunk >= 0; ++i) {
                ...
                if (Config()->m_SvRconPassword[0] == 0 && Config()->m_SvRconModPassword[0] == 0) {
                    if (!m_aClients[ClientID].m_NoRconNote) {
                        SendRconLine(ClientID,
                                     "No rcon password set on server. Set sv_rcon_password and/or sv_rcon_mod_password to enable the remote console.");
                    ...
                    m_aClients[ClientID].m_pRconCmdToSend = Console()->FirstCommandInfo(IConsole::ACCESS_LEVEL_ADMIN,
                                                                                        CFGFLAG_SERVER);
                    ...
                    m_aClients[ClientID].m_pRconCmdToSend = Console()->FirstCommandInfo(IConsole::ACCESS_LEVEL_MOD,
                                                                                        CFGFLAG_SERVER);
                    SendRconLine(ClientID,
                    ...
                    char aBuf[128];
                    str_format(aBuf, sizeof(aBuf), "Wrong password %d/%d.", m_aClients[ClientID].m_AuthTries,
                               Config()->m_SvRconMaxTries);
                    SendRconLine(ClientID, aBuf);
                    if (m_aClients[ClientID].m_AuthTries >= Config()->m_SvRconMaxTries) {
                        if (!Config()->m_SvRconBantime)
                            m_NetServer.Drop(ClientID, "Too many remote console authentication tries");
                        else
                            m_ServerBan.BanAddr(m_NetServer.ClientAddr(ClientID), Config()->m_SvRconBantime * 60,
            ...
            if (Config()->m_Debug) {
                char aBuf[256];
                str_format(aBuf, sizeof(aBuf), "strange message ClientID=%d msg=%d data_size=%d", ClientID,
                           Unpacker.Type(), pPacket->m_DataSize);
                Console()->Print(IConsole::OUTPUT_LEVEL_DEBUG, "server", aBuf);
    ...
    if (Token != -1) {
        for (int i = 0; i < MAX_CLIENTS; i++) {
            if (m_aClients[i].m_State != CClient::STATE_EMPTY) {
                ...
                pPacker->AddInt(GetClassID(m_aClients[i].m_PlayerClass.m_Class)); // client class ID
    ...
    while (m_NetServer.Recv(&Packet, &ResponseToken)) {
        if (Packet.m_Flags & NETSENDFLAG_CONNLESS) {
            ...
            if (Packet.m_DataSize >= int(sizeof(SERVERBROWSE_GETINFO)) &&
                ...
                CUnpacker Unpacker;
                Unpacker.Reset((unsigned char *) Packet.m_pData + sizeof(SERVERBROWSE_GETINFO),
                               Packet.m_DataSize - sizeof(SERVERBROWSE_GETINFO));
...
int CServer::LoadMap(const char *pMapName) {
    CMapData data;
    char aBufMultiMap[IO_MAX_PATH_LENGTH];
    for (int i = 0; i < (int) m_vMapData.size(); ++i) {
...
void CServer::InitRegister(CNetServer *pNetServer, IEngineMasterServer *pMasterServer, CConfig *pConfig,
                           IConsole *pConsole) {
...
int CServer::Run() {
    ...
    if (!m_NetServer.Open(BindAddr, Config(), Console(), Kernel()->RequestInterface<IEngine>(), &m_ServerBan,
                          Config()->m_SvMaxClients, Config()->m_SvMaxClientsPerIP, NewClientCallback, DelClientCallback,
                          this)) {
    ...
    // start game
    {
        ...
        while (m_RunServer) {
            ...
            for (int c = 0; c < MAX_CLIENTS; c++) {
                if (m_aClients[c].m_MapID != m_aClients[c].m_NextMapID ||
                    m_CurrentGameTick >= 0x6FFFFFFF) //	force reload to make sure the ticks stay within a valid range
                {
                    ChangeClientMap(c);
            ...
            m_NetServer.Wait(clamp(int((TickStartTime(m_CurrentGameTick + 1) - time_get()) * 1000 / time_freq()), 1,
        ...
        if (pThis->m_aClients[i].m_State != CClient::STATE_EMPTY) {
            ...
            if (pThis->m_aClients[i].m_State == CClient::STATE_INGAME) {
                ...
                str_format(aBuf, sizeof(aBuf), "id=%d addr=%s client=%x name='%s' score=%d %s", i, aAddrStr,
                           pThis->m_aClients[i].m_Version, pThis->m_aClients[i].m_aName, pThis->m_aClients[i].m_Score,
                           pAuthStr);
            } else
...
void CServer::DemoRecorder_HandleAutoStart() {
    if (Config()->m_SvAutoDemoRecord) {
        ...
        m_DemoRecorder.Start(aFilename, GameServer()->NetVersion(), m_vMapData[MAP_DEFAULT_ID].m_aCurrentMap,
                             m_vMapData[MAP_DEFAULT_ID].m_CurrentMapSha256, m_vMapData[MAP_DEFAULT_ID].m_CurrentMapCrc,
                             "server");
...
void CServer::ConRecord(IConsole::IResult *pResult, void *pUser) {
    ...
    pServer->m_DemoRecorder.Start(aFilename, pServer->GameServer()->NetVersion(),
                                  pServer->m_vMapData[MAP_DEFAULT_ID].m_aCurrentMap,
                                  pServer->m_vMapData[MAP_DEFAULT_ID].m_CurrentMapSha256,
                                  pServer->m_vMapData[MAP_DEFAULT_ID].m_CurrentMapCrc, "server");
}
...
void
CServer::ConchainSpecialInfoupdate(IConsole::IResult *pResult, void *pUserData, IConsole::FCommandCallback pfnCallback,
...
void
CServer::ConchainPlayerSlotsUpdate(IConsole::IResult *pResult, void *pUserData, IConsole::FCommandCallback pfnCallback,
                                   void *pCallbackUserData) {
    pfnCallback(pResult, pCallbackUserData);
    CServer *pSelf = (CServer *) pUserData;
    if (pResult->NumArguments()) {
...
void
CServer::ConchainMaxclientsUpdate(IConsole::IResult *pResult, void *pUserData, IConsole::FCommandCallback pfnCallback,
                                  void *pCallbackUserData) {
    pfnCallback(pResult, pCallbackUserData);
    CServer *pSelf = (CServer *) pUserData;
    if (pResult->NumArguments()) {
...
void CServer::ConchainMaxclientsperipUpdate(IConsole::IResult *pResult, void *pUserData,
...
void
CServer::ConchainModCommandUpdate(IConsole::IResult *pResult, void *pUserData, IConsole::FCommandCallback pfnCallback,
                                  void *pCallbackUserData) {
    if (pResult->NumArguments() == 2) {
        CServer *pThis = static_cast<CServer *>(pUserData);
        const IConsole::CCommandInfo *pInfo = pThis->Console()->GetCommandInfo(pResult->GetString(0), CFGFLAG_SERVER,
                                                                               false);
...
void CServer::ConchainConsoleOutputLevelUpdate(IConsole::IResult *pResult, void *pUserData,
                                               IConsole::FCommandCallback pfnCallback, void *pCallbackUserData) {
    pfnCallback(pResult, pCallbackUserData);
    if (pResult->NumArguments() == 1) {
        CServer *pThis = static_cast<CServer *>(pUserData);
...
void
CServer::ConchainRconPasswordSet(IConsole::IResult *pResult, void *pUserData, IConsole::FCommandCallback pfnCallback,
                                 void *pCallbackUserData) {
    pfnCallback(pResult, pCallbackUserData);
    if (pResult->NumArguments() >= 1) {
...
void CServer::RegisterCommands() {
    ...
    Console()->Register("kick", "i[id] ?r[reason]", CFGFLAG_SERVER, ConKick, this,
    ...
    Console()->Register("set_map_by_mapname", "s?i", CFGFLAG_SERVER, ConSetMapByName, this,
                        "Set <mapname> [<playerid>]");
